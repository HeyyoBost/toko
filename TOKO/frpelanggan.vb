Public Class frpelanggan
    ...
    End Sub
    Sub aturdatagrid()
        dgv.Columns(0).HeaderText = "Kode Pelanggan "
    ...
    Private Function getrowscount() As Integer
        tampilkan("select * from Pelanggan")
        ds = New DataSet
        mda.Fill(ds)
    ...
    Sub autokode()
        ...
        eksekusi.Connection = conn
        eksekusi.CommandType = CommandType.Text
        eksekusi.CommandText = "select * from Pelanggan order by kodepel DESC limit 1"
        cek = eksekusi.ExecuteReader()
        cek.Read()
        If cek.HasRows Then
            kode = cek.Item("kodepel")
            no = Val(Microsoft.VisualBasic.Right(kode, 3))
            no = no + 1
            kodebaru = "PEL-" + Format(no, "000")
            tkode.Text = kodebaru
        Else
            tkode.Text = "PEL-" + "001"
        End If
    ...
    Private Sub frpelanggan_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        dgv.Columns.Clear()
        tampilkan("select * from pelanggan")
        aturdatagrid()
        tombol()
    ...
    Private Sub bsimpan_Click(sender As Object, e As EventArgs) Handles bsimpan.Click
        ...
        ElseIf bsimpan.Text = "SIMPAN" Then
            ...
            eksekusi.Connection = conn
            eksekusi.CommandType = CommandType.Text
            eksekusi.CommandText = "insert into pelanggan (kodepel, namapel, emailpel, alamat,telp) VALUES ('" & tkode.Text & "','" & tnama.Text & "','" & temail.Text & "','" & talamat.Text & "','" & tnomer.Text & "')"
        ...
        Else
            ...
            eksekusi.Connection = conn
            eksekusi.CommandType = CommandType.Text
            eksekusi.CommandText = "update pelanggan set namapel= '" & tnama.Text & "',emailpel= '" & temail.Text & "',alamat='" & talamat.Text & "',telp='" & tnomer.Text & "' where kodepel= '" & tkode.Text & "' "
            eksekusi.ExecuteNonQuery()
    ...
    Private Sub dgv_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv.CellContentClick
        ...
        If e.ColumnIndex = 6 Then
            Dim x As Byte
            i = dgv.CurrentRow.Index
            x = MsgBox("hapus data  pelanggan dengan kode " & kode & " ?", MsgBoxStyle.Critical + vbYesNo, "konfirmasi")
            If x = vbYes Then
                cekkoneksi()
                eksekusi.Connection = conn
                eksekusi.CommandType = CommandType.Text
                eksekusi.CommandText = "delete from pelanggan where kodepel = '" & kode & "'"
                eksekusi.ExecuteNonQuery()
    ...
    Private Sub tkode_TextChanged(sender As Object, e As EventArgs) Handles tkode.TextChanged
        ...
        eksekusi.Connection = conn
        eksekusi.CommandType = CommandType.Text
        eksekusi.CommandText = "select * from pelanggan where kodepel = '" & tkode.Text & "'"
        cek = eksekusi.ExecuteReader
        cek.Read()
        If cek.HasRows Then
            tnama.Text = cek.Item("namapel")
            temail.Text = cek.Item("emailpel")
            talamat.Text = cek.Item("alamat")
    ...
    Private Sub tcari_TextChanged(sender As Object, e As EventArgs) Handles tcari.TextChanged
        dgv.Columns.Clear()
        tampilkan("select * from pelanggan where kodepel like '%" & tcari.Text & "%' or namapel like '%" & tcari.Text & "%' ")
        aturdatagrid()
        tombol()
    End Sub
    ...
    Private Sub tnomer_TextChanged(sender As Object, e As EventArgs) Handles tnomer.TextChanged
